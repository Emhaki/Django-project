...
1. 새로운 model 설정과 url 설정을 재정비, 트리 pk를 중심으로 데이터를 적재할 생각.
2. 여러가지 만져보고 url도 재정비 해봤지만, 저장된 데이터가 user pk와 tree pk가 같음
![](./video/20221103.png)
3. 현재 다른 프로젝트로 시간적 여유가 없지만, 다른 프로젝트에서 배운 내용을 토대로 개인 프로젝트에 적용해볼 생각